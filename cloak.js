class ABC {
			...
			open() {
				if (this.type == "blank") {
					...
					try {
						document.body.innerHTML = `<script>window.onhashchange = function() {if (window.innerDocClick) {window.innerDocClick = false;} else {if (window.location.hash != '#undefined') {alert("back");} else {history.pushState("", document.title, window.location.pathname);location.reload();}}}</script>
	<iframe
